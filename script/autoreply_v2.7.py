...
# -*- coding: utf-8 -*-
"""
Autoreply-Mailfilter fuer Postfix, gehärtete Patch Version 2.7
...
def load_json(path: str, missing_message: str) -> dict:
    ...
    try:
        ...
        if not isinstance(data, dict):
            print(f"Fehler: {path} muss ein JSON Objekt enthalten.")
            sys.exit(1)
        return data
    except Exception as e:
...
def reinject_email(message_bytes: bytes, sender: str, recipients: List[str], original_id: Optional[str]) -> bool:
...
def _load_attachment_from_cfg(server_settings: dict, recipient_config: dict) -> Optional[Tuple[str, str, str, bytes]]:
    ...
    try:
        ...
        if not allowed_dir_r or not os.path.isdir(allowed_dir_r):
            log_error(f"ATTACH_BLOCKED reason=invalid_allowed_dir dir={allowed_dir}")
            return None
        ...
        if not os.path.isfile(p_r):
            log_error(f"ATTACH_BLOCKED path={cfg_att} reason=not_a_file")
            return None
...
class _SMTPDirectIP(smtplib.SMTP):
    def __init__(self, target_ip: str, *args, **kwargs):
...
def send_email(message: EmailMessage, server_settings: dict) -> bool:
    try:
...
def validate_server_config(server_settings: dict) -> None:
    ...
    max_message_bytes = _safe_int(server_settings.get('max_message_bytes', 25 * 1024 * 1024), 25 * 1024 * 1024, minimum=1024, maximum=100 * 1024 * 1024)
    server_settings['max_message_bytes'] = max_message_bytes

    max_attachment_bytes = _safe_int(server_settings.get('max_attachment_bytes', 10 * 1024 * 1024), 10 * 1024 * 1024, minimum=1, maximum=100 * 1024 * 1024)
...
def validate_user_config(user_settings: dict) -> None:
    ...
    for idx, rule in enumerate(rules):
        if not isinstance(rule, dict):
            raise ValueError(f"user config: autoreply[{idx}] ist kein Objekt")

        has_email = 'email' in rule
        has_domain = 'domain' in rule
        if not has_email and not has_domain:
            raise ValueError(f"user config: autoreply[{idx}] braucht email oder domain")

        if has_email:
            emails = _as_list(rule.get('email'))
            emails_n = normalize_email_list([str(x or "") for x in emails])
            if not emails_n:
                raise ValueError(f"user config: autoreply[{idx}] email ist leer oder ungueltig")
            rule['email'] = emails_n
        ...
        if has_domain:
            ...
            for d in domains:
                ...
                if dn:
                    domains_n.append(dn)
            if not domains_n:
                raise ValueError(f"user config: autoreply[{idx}] domain ist leer oder ungueltig")
            rule['domain'] = domains_n

        if 'blacklist' in rule and not isinstance(rule.get('blacklist'), list):
            raise ValueError(f"user config: autoreply[{idx}] blacklist muss Liste sein")
        ...
        for key in ('body', 'subject', 'from', 'reply-to', 'attachment_path'):
            if key in rule and rule.get(key) is not None and not isinstance(rule.get(key), str):
                raise ValueError(f"user config: autoreply[{idx}] {key} muss String sein")
...
def main() -> None:
    ...
    max_bytes = _safe_int(server_settings.get("max_message_bytes", 25 * 1024 * 1024), 25 * 1024 * 1024, minimum=1024, maximum=100 * 1024 * 1024)
    ...
    if integration_mode != "bcc":
        ...
        if ok:
            ...
            return
        log_error("Integration Mode klassisch, Reinject fehlgeschlagen")
        sys.exit(1)
